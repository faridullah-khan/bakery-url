...
// Firestore client via Firebase Admin using env-based service account for Render (falls back to ADC)
const {
  FIREBASE_PROJECT_ID,
...
// Inventory routes
// POS visibility: cashiers can read inventory only
...
// Inventory mutations: admin/manager only
...
// Recipes: calculate cost based on ingredients and inventory prices
...
// POS flow: cashiers can create sales
    ...
    const result = await firestore.runTransaction(async (tx) => {
      ...
      for (const item of items) {
        ...
        for (const ing of ingredients) {
          ...
          // cost per unit uses recipe quantity (not multiplied by sale qty)
